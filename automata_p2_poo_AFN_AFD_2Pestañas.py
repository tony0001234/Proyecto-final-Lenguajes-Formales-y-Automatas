...
class Matriz:
    ...
    def _actualizar_afd(self):
        ...
        # Preparar datos para el Treeview AFD
        estados_afd = sorted(set(estado for estado, _ in afd.keys()) | {frozenset()}, key=lambda x: ','.join(sorted(x)) if x else '0')
        simbolos = sorted(alfabeto - {'e'})

        # Configurar columnas para AFD
        self.tree_afd["columns"] = ["Estado"] + simbolos
        self.tree_afd.column("Estado", width=200, anchor="center")
        ...
        for estado in estados_afd:
            valores = [','.join(sorted(estado)) if estado else '0']  # Representar el estado vacío como '0'
            for simbolo in simbolos:
                destino = afd.get((estado, simbolo), frozenset())
                valores.append(','.join(sorted(destino)) if destino else '0')
            self.tree_afd.insert("", "end", values=valores)

        # Print debug information
        print("AFD states:", estados_afd)
        print("AFD transitions:", afd)
        print("AFD final states:", estados_finales_afd)
